...
从零部署一个机器人实例的完整流程（环境准备 → 部署 → 扫码登录 → 启动 → 开启插件 → Web 面板），
请查看 **[📖 使用教程 (USAGE.md)](USAGE.md)**。

快速上手只需两步：

```bash
# 1. 运行安装脚本（自动保存到 ~/napbot/）
bash <(curl -s https://raw.githubusercontent.com/Bdlxx/NapCat-WordLibBot/master/install.sh)

# 2. 主菜单选 1 部署新实例 → 输入机器人 QQ → 按向导填写昵称和主人 QQ → 扫码登录 → 启动 Bot
```

> 部署时脚本会询问**机器人昵称**和**主人 QQ**（必填，多个逗号分隔），自动生成 `config.json`。
