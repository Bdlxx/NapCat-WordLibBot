...
def handle_marriage(event, target_qq=None):
    ...
    if not group_id:
        send_message(event, "该功能只能在群内使用。")
        return True

    members = get_group_members(group_id)
    if not members:
        send_message(event, "无法获取群成员列表，请稍后再试。")
        return True
    ...
    if str(user_id) in marriage_data[group_key][today]:
        send_message(event, "你今天已经有对象了，不能再次结婚。")
        return True
    ...
    if target_qq is None:
        ...
        if not available:
            send_message(event, "群内所有其他成员今天都有对象了，明天再试吧。")
            return True
    ...
    if random.randint(1, 100) > success_rate:
        send_message(event, "表白失败，对方拒绝了你的求婚。")
        return True
    ...
    # 最终检查
    if str(user_id) in marriage_data[group_key][today]:
        send_message(event, "你刚刚已经结婚了，请稍后再试。")
        return True
    if str(target_qq) in marriage_data[group_key][today]:
        send_message(event, "对方刚刚有了对象，请稍后再试。")
        return True
...
def handle_divorce(event):
    ...
    if user_cd and user_cd > time.time():
        ...
        send_message(event, f"你刚刚离过婚，请等待 {int(hours)} 小时 {int(minutes)} 分钟后再离婚。")
        return True
    ...
    if group_key not in marriage_data or today not in marriage_data[group_key]:
        send_message(event, "你今天都没有对象，离什么婚？")
        return True
    ...
    send_message(event, "离婚成功，你可以重新开始新的恋情了。")
    return True
...
def handle_set_prob(event):
    if not is_master(event.get("user_id")):
        send_message(event, "只有主人才能设置概率。")
        return True
    parts = event.get("raw_message", "").split()
    if len(parts) != 2:
        send_message(event, "格式错误：设置结婚概率 数字")
        return True
    try:
        rate = int(parts[1])
        if 0 <= rate <= 100:
            ...
            save_config(config)
            send_message(event, f"结婚成功率已设置为 {rate}%")
        else:
            send_message(event, "概率必须在0-100之间。")
    except:
        send_message(event, "请输入数字。")
    return True

def handle_set_cd(event):
    if not is_master(event.get("user_id")):
        send_message(event, "只有主人才能设置CD。")
        return True
    parts = event.get("raw_message", "").split()
    if len(parts) != 2:
        send_message(event, "格式错误：设置离婚CD 小时")
        return True
    try:
        hours = float(parts[1])
        if hours >= 0:
            ...
            save_config(config)
            send_message(event, f"离婚CD已设置为 {hours} 小时")
        else:
            send_message(event, "CD必须是非负数。")
    except:
        send_message(event, "请输入数字。")
    return True
...
def handle(event):
    ...
    if raw_msg.startswith("设置结婚概率"):
        return handle_set_prob(event)
    if raw_msg.startswith("设置离婚CD"):
    ...
    if raw_msg.startswith("娶") and at_qq is not None:
        return handle_marriage(event, at_qq)
    if raw_msg.startswith("嫁") and at_qq is not None:
        return handle_marriage(event, at_qq)
